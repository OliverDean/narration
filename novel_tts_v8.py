#!/usr/bin/env python3
"""
Advanced Novel TTS (Version 8):
 - Builds on Version 7 with an additional check for mismatched WAV parameters.
 - If a chunk's WAV has zero frames or different format than the first chunk, we remove it and skip.
...
Features:
  ...
  3) Improved speaker detection ("Name said" patterns, local entities).
  4) Round-robin speaker assignment if the text has more unique speakers than the TTS model supports.
  5) Dedicated "Narrator" voice + optional metadata in JSON.
  6) Optionally exclude characters (skip or reassign to narrator).
  7) Avoid splitting words mid-way by chunking at whitespace near max_chars.
  8) Skips empty or punctuation-only lines => no weird short WAVs.
  9) **Now** also checks each chunk's WAV for zero frames or param mismatch with the first chunk.

Usage Examples:
  python novel_tts_v8.py story.txt --model_name tts_models/en/vctk/vits --gpu
  python novel_tts_v8.py --batch chapter1.txt chapter2.txt ...
...
VOICE_MAP_FILE = "character_voices_v8.json"

def load_voice_map() -> dict:
    """
    Load or create a global JSON map: 
      { 'CharacterName': { 'speaker_id': 'p234', 'description': '...' }, ... }
    """
...
##############################################################################
# 8) Main Script: Version 8
##############################################################################

def main():
    parser = argparse.ArgumentParser(description="Advanced Novel TTS (v8) with fastcoref, param-checking, skip zero/mismatch WAVs.")
    ...
    for file_path in input_files:
        ...
        for paragraph in paragraphs:
            ...
            for (quote_text, q_start, q_end) in spans:
                ...
                if speaker is not None and speaker not in args.exclude_characters:
        ...
        # (C) Generate TTS
        temp_files = []
        first_params = None  # track the format of the first valid chunk
        try:
            for idx, (speaker_dict, line_text) in enumerate(final_lines):
                ...
                for cid, chunk in enumerate(chunks):
                    ...
                    print(f"[DEBUG] line={idx+1}, chunk={cid+1}, speaker={speaker_str}, text={repr(chunk_stripped)}")
                    ...
                    try:
                        tts.tts_to_file(text=chunk_stripped, speaker=speaker_str, file_path=tmp_wav_name)

                        # Now open the wav, read frames & params
                        ...
                        if n_frames == 0:
                            ...
                            os.remove(tmp_wav_name)  # file handle is closed now, safe to remove
                            continue

                        if first_params is None:
                            first_params = current_params
                        else:
                            if current_params != first_params:
                                print(f"[SKIP] Mismatched WAV params at line {idx+1}, chunk {cid+1}. Removing.")
            ...
            if temp_files:
                ...
                print("[INFO] No valid WAV files to combine. Skipped all chunks?")
